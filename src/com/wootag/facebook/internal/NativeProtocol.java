...
public final class NativeProtocol {
    ...
    private static final List<Integer> KNOWN_PROTOCOL_VERSIONS = Arrays.asList(
            Integer.valueOf(PROTOCOL_VERSION_20140324), Integer.valueOf(PROTOCOL_VERSION_20140204),
            Integer.valueOf(PROTOCOL_VERSION_20131107), Integer.valueOf(PROTOCOL_VERSION_20130618),
            Integer.valueOf(PROTOCOL_VERSION_20130502), Integer.valueOf(PROTOCOL_VERSION_20121101));

    public static Intent createPlatformActivityIntent(final Context context, final String action, final int version,
            final Bundle extras) {

        final Intent intent = findActivityIntent(context, INTENT_ACTION_PLATFORM_ACTIVITY, action);
        if (intent == null) {
            return null;
        }
    ...
    public static Intent createPlatformServiceIntent(final Context context) {
        ...
        for (final NativeAppInfo appInfo : facebookAppInfoList) {
    ...
    public static Intent createProxyAuthIntent(final Context context, final String applicationId,
            final List<String> permissions, final String e2e, final boolean isRerequest) {
    ...
    public static Exception getErrorFromResult(final Intent resultIntent) {

        if (!isErrorResult(resultIntent)) {
            return null;
        }

        final String type = resultIntent.getStringExtra(STATUS_ERROR_TYPE);
        final String description = resultIntent.getStringExtra(STATUS_ERROR_DESCRIPTION);

        if (ERROR_USER_CANCELED.equalsIgnoreCase(type)) {
            return new FacebookOperationCanceledException(description);
        }
        return new FacebookException(description);
    }

    public static int getLatestAvailableProtocolVersionForAction(final Context context, final String action,
            final int minimumVersion) {

        final List<NativeAppInfo> appInfoList = actionToAppInfoMap.get(action);
        return getLatestAvailableProtocolVersionForAppInfoList(context, appInfoList, minimumVersion);
    }

    public static int getLatestAvailableProtocolVersionForService(final Context context, final int minimumVersion) {
    ...
    public static boolean isErrorResult(final Intent resultIntent) {
    ...
    private static Map<String, List<NativeAppInfo>> buildActionToAppInfoMap() {
        ...
        final Map<String, List<NativeAppInfo>> map = new HashMap<String, List<NativeAppInfo>>();

        final ArrayList<NativeAppInfo> messengerAppInfoList = new ArrayList<NativeAppInfo>();
    ...
    private static List<NativeAppInfo> buildFacebookAppList() {
        ...
        final List<NativeAppInfo> list = new ArrayList<NativeAppInfo>();
    ...
    private static Uri buildPlatformProviderVersionURI(final NativeAppInfo appInfo) {

        return Uri.parse(CONTENT_SCHEME + appInfo.getPackage() + PLATFORM_PROVIDER_VERSIONS);
    }

    private static Intent findActivityIntent(final Context context, final String activityAction,
            final String internalAction) {

        final List<NativeAppInfo> list = actionToAppInfoMap.get(internalAction);
        if (list == null) {
            return null;
        }

        Intent intent = null;
        for (final NativeAppInfo appInfo : list) {
    ...
    private static int getLatestAvailableProtocolVersionForAppInfo(final Context context, final NativeAppInfo appInfo,
            final int minimumVersion) {

        final ContentResolver contentResolver = context.getContentResolver();

        final String[] projection = new String[] { PLATFORM_PROVIDER_VERSION_COLUMN };
        final Uri uri = buildPlatformProviderVersionURI(appInfo);
        Cursor cursor = null;
        try {
            ...
            final Set<Integer> versions = new HashSet<Integer>();
            while (cursor.moveToNext()) {
                final int version = cursor.getInt(cursor.getColumnIndex(PLATFORM_PROVIDER_VERSION_COLUMN));
                versions.add(Integer.valueOf(version));
            }

            for (final Integer knownVersion : KNOWN_PROTOCOL_VERSIONS) {
    ...
    private static int getLatestAvailableProtocolVersionForAppInfoList(final Context context,
            final List<NativeAppInfo> appInfoList, final int minimumVersion) {
        ...
        for (final NativeAppInfo appInfo : appInfoList) {
            final int protocolVersion = getLatestAvailableProtocolVersionForAppInfo(context, appInfo, minimumVersion);
    ...
    static Intent validateActivityIntent(final Context context, final Intent intent, final NativeAppInfo appInfo) {
    ...
    static Intent validateServiceIntent(final Context context, final Intent intent, final NativeAppInfo appInfo) {

        if (intent == null) {
            return null;
        }

        final ResolveInfo resolveInfo = context.getPackageManager().resolveService(intent, 0);
        if (resolveInfo == null) {
            return null;
        }
    ...
    private static abstract class NativeAppInfo {
        ...
        private static final Set<String> validAppSignatureHashes = buildAppSignatureHashes();

        private static Set<String> buildAppSignatureHashes() {
        ...
        public boolean validateSignature(final Context context, final String packageName) {
            ...
            for (final Signature signature : packageInfo.signatures) {
                final String hashedSignature = Utility.sha1hash(signature.toByteArray());
